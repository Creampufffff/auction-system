...
@DisplayName("Kiểm thử Tầng Service của Đấu giá Tự động (AutoBidServiceImpl)")
class AutoBidServiceImplTest {
    ...
    @Test
    @DisplayName("createAutoBid: Tạo thành công lệnh đấu giá tự động với dữ liệu hợp lệ")
    void testCreateAutoBid_ValidData_Success() {
        ...
        assertTrue(savedBid.isActive(), "Lệnh AutoBid mới tạo phải ở trạng thái kích hoạt");
        assertEquals(500.0, savedBid.getMaxAutoAmount());
    }

    @Test
    @DisplayName("createAutoBid: Ném lỗi InvalidBidException nếu không tìm thấy phiên đấu giá")
    ...
    @Test
    @DisplayName("createAutoBid: Ném lỗi InvalidBidException nếu ngân sách thiết lập <= 0")
    ...
    @Test
    @DisplayName("cancelAutoBid: Vô hiệu hóa lệnh đấu giá tự động (isActive = false)")
    ...
    @Test
    @DisplayName("processAutoBidsForAuction: Tự động đặt giá thành công nếu còn đủ ngân sách")
    void testProcessAutoBids_SufficientFunds_PlacesBidSuccessfully() {
        ...
        assertEquals(210.0, placedBid.getBidAmount(), "Mức giá tự động phải bằng giá cao nhất + bước giá (200 + 10 = 210)");
        assertTrue(autoBid.isActive(), "AutoBid vẫn đủ tiền nên phải giữ trạng thái active");
    }

    @Test
    @DisplayName("processAutoBidsForAuction: Hủy AutoBid nếu mức giá tiếp theo vượt quá ngân sách cho phép")
    ...
    @Test
    @DisplayName("getAutoBiddsByAuctionId: Hàng đợi ưu tiên xử lý lệnh có ngân sách cao nhất trước")
    ...
    @Test
    @DisplayName("processAutoBidsForAuction: Tự động vô hiệu hóa lệnh nếu gặp sự cố Database khi đặt giá")
